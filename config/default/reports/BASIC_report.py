...
if 'config.default.reports.BASIC' in sys.modules:
    del sys.modules['config.default.reports.BASIC']

from config.default.reports import BASIC
...
if get_report:
    try:
        ...
        QApplication.processEvents()
        BASIC = BASIC(widgets, options, directoryName, self.config)
        for wafer in BASIC.wafers:
            dir_wafer = directoryName + "/" + wafer
            dir_data = dir_wafer + "/" + options["data_folder"]
            filename_results = dir_wafer + "/" + wafer + ".dat"
            widgets.txtResultReport.appendPlainText("Creating report BASIC for wafer " + wafer + " ...")
            QApplication.processEvents()
            result_file = ResultFile(filename_results)
            # Make pdf report for every wafer
            BASIC.print_report(wafer)
        # print comparative
        BASIC.print_comparative()

        BASIC.output(filename_pdf)
        widgets.txtResultReport.appendPlainText("Merging files...")
        QApplication.processEvents()
        filename_annex = "G:\\GITHUB\\Python\\caracterizar2\\config\\default\\reports\\assets\\annexUPC.pdf"
        merger = PdfMerger()
