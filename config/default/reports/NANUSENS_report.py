...
if 'config.default.reports.NANUSENS' in sys.modules:
    del sys.modules['config.default.reports.NANUSENS']

from config.default.reports import NANUSENS
...
if get_report:
    try:
        ...
        QApplication.processEvents()
        NANUSENS = NANUSENS(widgets, options, directoryName, self.config)
        ...
        QApplication.processEvents()
        filename_annex = "C:\\GITHUB\\Python\\caracterizar2\\config\\default\\reports\\assets\\annexNANUSENS.pdf"
        merger = PdfMerger()
