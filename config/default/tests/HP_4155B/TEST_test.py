...
if cartographic_measurement:
	...
	if test_status.status=="STARTED":	
		...
		self.waferwindow.meas_result[int(dieActual)-1][int(moduleActual)-1] = {
		    ...
		    "variables" : {
	    		"params" : [{"name" : "CMAX", "value" : 420.056E-12},{"name" : "CMIN", "value" : 210.057E-12}],
	       		"data" : [{"name" : "V", "values" : [0,1,2,3,4,5], "units" : "V"},{"name": "C", "values" : [210.057E-12, 215.24E-12, 310.85E-12, 360.45E-12, 400E-12, 420.56E-12], "units": "F"}]
	     	},
		...
		#self.waferwindow.meas_result[int(dieActual)-1][int(moduleActual)-1]["status"]= meas_status

		#print("Die: "+dieActual+", Module: "+moduleActual)
		
		# if self.waferwindow.total_meas()["meas_warning"]>20:
