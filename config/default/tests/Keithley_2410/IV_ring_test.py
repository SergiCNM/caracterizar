# Test IV_ring in two Keithley 2410 instruments
import os.path
...
def measure_IV_ring(k2410_pad, k2410_ring, IV_ring_parameters, main):
    voltage_list = []
    current_pad_list = []
    current_ring_list = []
    resistance_pad = []
    ...
    # Enable outputs
    k2410_pad.set_voltage(start)
    k2410_ring.set_voltage(start)
    k2410_pad.output("ON")
    ...
    for V in voltages:
        try:
            ...
            k2410_pad.set_voltage(V)
            k2410_ring.set_voltage(V)

            time.sleep(delay)

            # Measure currents
            I_pad = float(k2410_pad.measure_current_once())
            I_ring = float(k2410_ring.measure_current_once())
        ...
        if cartographic_measurement:
            ...
            if test_status.status == "STARTED":
                ...
                if abs(float(IV_ring_parameters["STOP"]) - voltage_end) < 1e-6:
                    ...
                    if k2410_pad.get_error_count() == 0 and k2410_ring.get_error_count() == 0:
