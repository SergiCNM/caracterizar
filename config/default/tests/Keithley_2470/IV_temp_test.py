...
from config.default.probers import MPI_TS2000SE
...
def save_file(self, voltage, current, namefile):
    """
    Save file
...
def save_log_file(self, temp_value, res_value):
    ...
    f = open(namefile, "w+")
    f.write(str(temp_value) + "," + str(res_value) + "," + str(d1) + " " + str(current_time))
    f.close()
...
def measure_IV(IV_parameters, k2470):
    ...
    sense_float = []
    if len(sense)>0:
        source_float = list(map(float, source))
        sense_float = list(map(float, sense))
    return [source_float, sense_float]
...
if __name__ == "__main__":
    ...
    MPI = MPI_TS2000SE(probers["MPI_TS2000SE"])
    # init IV_parameters
    load_IV_parameters()
    print(IV_parameters)
    temp_list = IV_parameters["TEMP_LIST"].split(",")

    # make measurements
    actual_temp = float(MPI.get_chuck_temp())
    print(f"make measurements temp {actual_temp}")

    source, sense = measure_IV(IV_parameters, k2470)
    print(f"Source: {source}")
    print(f"Sense: {sense}")
    # self.updateTextDescription(txt_result)
    plot_parameters = get_plot_parameters(source, sense)
    ...
    #     MPI.set_chuck_temp(temp)
    #     margin_top_temp = float(temp) + (float(temp) * 1/100)
    #     margin_bottom_temp = float(temp) - (float(temp) * 1/100)
    #     actual_temp = float(MPI.get_chuck_temp())
    #     while(actual_temp > margin_top_temp or actual_temp < margin_bottom_temp
    #           or MPI.get_chuck_thermo_state() != "Controlling"):
    #         actual_temp = float(MPI.get_chuck_temp())
    #         print("waiting 10 seconds...")
    #         time.sleep(10)
    #
    #     # extra wait time
    #     time.sleep(int(IV_parameters["STABLE_TIME"]))
    #     # make measurements
    #     actual_temp = float(MPI.get_chuck_temp())
    #     print(f"make measurements temp {actual_temp}")
    ...
    #         self.show_graph(plot_parameters)
    #         res_value = 10
    #         save_log_file(actual_temp, res_value)
    ...
    #             }
    #
    #             namefile = f"{self.ui.txtLot.text()}_W" + f"{int(self.ui.txtWafer.text()):02d}_{str(dieActual)}_{str(moduleActual)}"
    #             save_file(self, voltage, current, namefile)
    #     else:
    #         # init IV_parameters
    #         load_IV_parameters()
    #         # single measure
    ...
    #             namefile = f"{self.ui.txtLot.text()}_W{int(self.ui.txtWafer.text()):02d}_0_1"
    #             save_file(self, voltage, current, namefile)
