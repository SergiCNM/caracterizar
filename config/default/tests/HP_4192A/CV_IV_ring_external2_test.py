# -------------------------------------------------------
# Test CV_IV_ring_external2 in HP 4192A and two Keithley 2410 instruments
# -------------------------------------------------------
# This test is used to measure CV curves using two external voltage sources
# - Two Keithley 2410 apply the same voltage steps (pad and ring)
...
def config_HP4192A_for_spot_measurement(hp4192A, CV_IV_ring_external_parameters):
    ...
    The DC bias is assumed to be provided by external voltage sources (Keithley 2410 pad and ring),
...
def measure_CV_IV_ring_external(main, k2410_pad, k2410_ring, hp4192A, CV_IV_ring_external_parameters):
    """
    Measure CV curve using two external voltage sources (Keithley 2410 pad and ring)
    Both SMUs apply the same voltage simultaneously
    :param main: main program instance
    :param k2410_pad: Keithley 2410 instrument instance for pad
    :param k2410_ring: Keithley 2410 instrument instance for ring
    :param hp4192A: HP_4192A instrument instance
    :param CV_IV_ring_external_parameters: parameters dictionary
    ...
    capacitance_list = []
    conductance_list = []
    print("measuring CV_IV_ring_external2 with HP_4192A...")
    ...
    # Configure both Keithley 2410 for voltage source mode
    if k2410_pad.config_IV(CV_IV_ring_external_parameters) and k2410_ring.config_IV(CV_IV_ring_external_parameters):
        k2410_pad.set_voltage(start_v)
        k2410_ring.set_voltage(start_v)
        k2410_pad.output("ON")
        k2410_ring.output("ON")
    else:
        print("Error configuring Keithley 2410 instruments")
        return [], [], [], [], []
    ...
    for voltage in voltage_steps:
        try:
            # Set voltage on both Keithley 2410 (same voltage on pad and ring)
            print("Setting voltage to:", voltage)
            k2410_pad.set_voltage(voltage)
            k2410_ring.set_voltage(voltage)

            # Wait for voltage to settle
            if CV_IV_ring_external_parameters["SETTLE_TIME"] > 0:
                time.sleep(CV_IV_ring_external_parameters["SETTLE_TIME"])

            # Measure current with both Keithley 2410
            current_pad = k2410_pad.measure_current_once()
            current_ring = k2410_ring.measure_current_once()
    ...
    k2410_pad.output("OFF")
    k2410_ring.output("OFF")
...
try:
    ...
    k2410_pad = Keithley_2410(instruments["Keithley_2410"])
    ...
    if cartographic_measurement:
        if str(dieActual) == "1" and str(moduleActual) == "1":
            retval = message_user(
                main,
                "Init instruments for CV_IV_ring_external2 (HP_4192A)!",
            ...
            for freq in freqs:
                ...
                if config_HP4192A_for_spot_measurement(hp4192A, CV_IV_ring_external_parameters):
                    ...
                    voltage, current_pad, current_ring, capacitance, conductance = measure_CV_IV_ring_external(
                        main, k2410_pad, k2410_ring, hp4192A, CV_IV_ring_external_parameters
                    )
                    ...
                    main.waferwindow.meas_result[int(dieActual) - 1][int(moduleActual) - 1] = {
                        ...
                        "plot_parameters": {
                            "name": f"Plot CV_IV_ring_external2 HP_4192A {freq}kHz (Die {dieActual} Module {moduleActual})",
                            "x": voltage.tolist(),
                            "y1": (capacitance * 1e12).tolist(),
                            "y2": (conductance * 1e9).tolist(),
                            "titles": {
                                "title": f"CV_IV_ring_external2 HP_4192A {freq}kHz (Die {dieActual} Module {moduleActual})",
                                "left": "Capacitance",
                                "bottom": "Voltage",
                                "right": "Conductance",
                            },
                    ...
                    namefile = (
                        ...
                        + f"/CV_IV_ring_external2_HP_4192A_{freq}kHz_{main.ui.txtProcess.text()}_{dieActual}_{moduleActual}.txt"
                    )
        ...
        for freq in freqs:
            ...
            if config_HP4192A_for_spot_measurement(hp4192A, CV_IV_ring_external_parameters):
                ...
                if len(voltage) > 0:
                    ...
                    plot_parameters = {
                        "name": f"Plot CV_IV_ring_external2 HP_4192A {freq}kHz",
                        "x": voltage.tolist(),
                        "y1": (capacitance * 1e12).tolist(),
                        "y2": (conductance * 1e9).tolist(),
                        "titles": {
                            "title": f"CV_IV_ring_external2 HP_4192A Measurement at {freq}kHz",
                            "left": "Capacitance",
                            "bottom": "Voltage",
                            "right": "Conductance",
                        },
                    ...
                    namefile = (
                        main.getDirs("results")
                        + f"/CV_IV_ring_external2_HP_4192A_{freq}kHz_{main.ui.txtProcess.text()}_single.txt"
                    )
                    main.save_lists_to_txt(
                        namefile=namefile,
                        var_list=[voltage, current_pad, current_ring, capacitance, conductance],
