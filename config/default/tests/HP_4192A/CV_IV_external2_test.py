# -------------------------------------------------------
# Test CV_IV_external2 in HP 4192A and Keithley 2410 instruments
# -------------------------------------------------------
# This test is used to measure CV curves using an external voltage source
# - Keithley 2410 applies voltage steps
...
def config_HP4192A_for_spot_measurement(hp4192A, CV_IV_external_parameters):
    ...
    The DC bias is assumed to be provided by an external voltage source (Keithley 2410),
...
def measure_CV_IV_external(main, k2410, hp4192A, CV_IV_external_parameters):
    """
    Measure CV curve using external voltage source (Keithley 2410)
    :param main: main program instance
    :param k2410: Keithley 2410 instrument instance
    :param hp4192A: HP_4192A instrument instance
    :param CV_IV_external_parameters: parameters dictionary
    ...
    # Configure Keithley 2410 for voltage source mode
    if k2410.config_IV(CV_IV_external_parameters):
        k2410.set_voltage(start_v)
        k2410.output("ON")
    else:
        print("Error configuring Keithley 2410")
        return [], [], [], []
...
try:
    ...
    k2410 = Keithley_2410(instruments["Keithley_2410"])
    ...
    if cartographic_measurement:
        if str(dieActual) == "1" and str(moduleActual) == "1":
            retval = message_user(
                main,
                "Init instruments for CV_IV_external2 (HP_4192A)!",
            ...
            for freq in freqs:
                ...
                if config_HP4192A_for_spot_measurement(hp4192A, CV_IV_external_parameters):
                    ...
                    voltage, current, capacitance, conductance = measure_CV_IV_external(
                        main, k2410, hp4192A, CV_IV_external_parameters
                    )
                    ...
                    main.waferwindow.meas_result[int(dieActual) - 1][int(moduleActual) - 1] = {
                        ...
                        "plot_parameters": {
                            "name": f"Plot CV_IV_external2 HP_4192A {freq}kHz (Die {dieActual} Module {moduleActual})",
                            "x": voltage.tolist(),
                            "y1": (capacitance * 1e12).tolist(),
                            "y2": (conductance * 1e9).tolist(),
                            "titles": {
                                "title": f"CV_IV_external2 HP_4192A {freq}kHz (Die {dieActual} Module {moduleActual})",
                                "left": "Capacitance",
                                "bottom": "Voltage",
                                "right": "Conductance",
                            },
                    ...
                    namefile = (
                        ...
                        + f"/CV_IV_external2_HP_4192A_{freq}kHz_{main.ui.txtProcess.text()}_{dieActual}_{moduleActual}.txt"
                    )
        ...
        for freq in freqs:
            ...
            if config_HP4192A_for_spot_measurement(hp4192A, CV_IV_external_parameters):
                ...
                if len(voltage) > 0:
                    ...
                    plot_parameters = {
                        "name": f"Plot CV_IV_external2 HP_4192A {freq}kHz",
                        "x": voltage.tolist(),
                        "y1": (capacitance * 1e12).tolist(),
                        "y2": (conductance * 1e9).tolist(),
                        "titles": {
                            "title": f"CV_IV_external2 HP_4192A Measurement at {freq}kHz",
                            "left": "Capacitance",
                            "bottom": "Voltage",
                            "right": "Conductance",
                        },
                    ...
                    namefile = (
                        main.getDirs("results")
                        + f"/CV_IV_external2_HP_4192A_{freq}kHz_{main.ui.txtProcess.text()}_single.txt"
                    )
                    main.save_lists_to_txt(
                        namefile=namefile,
                        var_list=[voltage, current, capacitance, conductance],
