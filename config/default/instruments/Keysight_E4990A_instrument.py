...
class Keysight_E4990A:
    ...
    def config_CV_RF(self, CV_parameters):
        """
        Configuración del E4990A para medida RF con SMU externo.
        - Bias interno siempre 0 V (la tensión la aplica el SMU).
        - Medida en modo continuo (INIT:CONT ON).
        - Usa parámetros de FREQ, OSC, APERTURE, promedios, etc.
        """

        try:
            # --- Display y parámetros a medir ---
            self.instrument.write(':DISP:WIND1:SPL D1_2')
            self.instrument.write(f':CALC1:PAR1:DEF {CV_parameters["GRAPH1"]}')
            self.instrument.write(f':CALC1:PAR2:DEF {CV_parameters["GRAPH2"]}')

            # --- Señal AC ---
            self.instrument.write(f':SOUR1:VOLT:LEV {CV_parameters["OSC"] * 1e-3}')  # en V
            self.instrument.write(f':SENS1:FREQ:CW {CV_parameters["FREQ"] * 1e3}')  # en Hz

            # Sweep type bias and number of points
            self.instrument.write(':SENS1:SWE:TYPE BIAS')
            self.instrument.write(f':SENS1:SWE:POIN {CV_parameters["NUM_POINTS"]}')
            self.instrument.write(f':SOUR1:BIAS:VOLT:STAR {str(CV_parameters["START_VOLTAGE"])}')  # Set Start Voltage
            self.instrument.write(f':SOUR1:BIAS:VOLT:STOP {str(CV_parameters["STOP_VOLTAGE"])}')  # Set Stop Voltage
            # STAT ON
            self.instrument.write(':SOUR1:BIAS:STAT ON') # Bias ON
            # APERTURE
            self.instrument.write(f':SENS1:APER {CV_parameters["APERTURE"]}')  # Set measurement speed
            self.instrument.write(f':SENS1:AVER:COUN {CV_parameters["AVERAGE_POINTS"]}')  # Set Point Averaging Count
            self.instrument.write(f':SENS1:AVER:STAT {1 if CV_parameters["POINT_AVERAGE"] else 0}')  # Enable/Disable Point Averaging
            ...
            print(f"Error in config_CV_RF: {ex}")
            return False
    ...
    def measure(self, parameters):
        try:
            ...
            opc = self.instrument.query('*OPC?') # increase timeout to wait measure finish
            self.autoscale()
            error = self.error()
            if error != '+0,"No error"':
                print(f"ERROR: {error}")
